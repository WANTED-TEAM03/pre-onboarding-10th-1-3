...
- **로컬스토리지를 다루기 위한 커스텀훅과 유틸함수 사용에 대해**
    - 커스텀훅을 사용 시 네트워크 요청이 일어나는 service단에서 훅을 사용하여 관리할 수 없었습니다.
    - 유틸함수로 만들어 사용하는 방식은 코드 분리의 의미가 크게 없다고 판단했습니다.
    - `커스텀훅과 유틸함수를 사용하지 않고 로컬스토리지 로직이 필요한 부분에서 localStorage 객체를 직접 참조하여 사용하는 방식으로 결정했습니다.`
- **Todo의 CRUD 비즈니스 로직을 util함수로 분리할 지 컴포넌트단에서 만들어 사용할 지에 관하여**
    - CRUD로직을 util함수로 분리하면 비즈니스 로직을 분리하여 컴포넌트 코드의 양을 줄일 수 있고 유지보수가 쉬워지며 단일 책임의 원칙을 지킬 수 있는 장점이 있습니다.
    - 컴포넌트단에 CRUD 로직을 구현하면 코드의 해당 state의 Setstate가 일어나는 과정을 한 눈에 볼 수 있어 코드의 가독성이 좋아 이해하기 쉽다는 장점이 있습니다.
    - `과제 특성 상 Depth가 깊지 않고 가독성 측면을 고려하여 컴포넌트단에서 투두의 CRUD 비즈니스 로직을 구현하도록 결정했습니다.`
